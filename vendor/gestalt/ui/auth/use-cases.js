...
const USE_CASE_COLUMNS = `
  id, slug, abp_id, short_id, uc_number, product_code, title, summary,
  description_why, description_what, description_bounds,
  actor, object_name, pre_condition, post_condition,
  status, visibility, body_md, vault_path, sort_order, updated_at
`.replace(/\s+/g, ' ').trim()
...
export async function fetchUseCasesByProduct(
  productCode,
  { publicOnly = false, withChildren = false, withRequirements = true } = {},
) {
  if (!isSupabaseConfigured()) return null

  const { data, error } = await buildUseCaseQuery(publicOnly).eq('product_code', productCode)
  if (error) throw new Error(error.message)
  const list = (data ?? []).map((row) => mapUseCaseRow(row))
...
export async function fetchAllUseCases({ publicOnly = false } = {}) {
  if (!isSupabaseConfigured()) return null
...
export async function fetchUseCaseBySlug(productCode, slug, { publicOnly = false } = {}) {
  if (!isSupabaseConfigured()) return null
