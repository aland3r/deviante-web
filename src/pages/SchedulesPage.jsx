...
export default function SchedulesPage() {
  ...
  const actions = <div className="flex rounded-lg border border-border overflow-hidden">{VIEWS.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setView(id)} className="flex items-center gap-1.5 px-3 py-2 text-[11px]" style={{ border: 0, background: view === id ? '#1e2738' : '#111520', color: view === id ? '#e2e8f0' : '#64748b' }}><Icon size={11} />{label}</button>)}</div>
