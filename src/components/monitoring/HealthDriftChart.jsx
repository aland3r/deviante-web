  ...
  return (
    <div className="flex-1 min-h-0">
      <div className="flex flex-wrap items-center justify-end gap-3 text-[10px] text-muted-foreground mb-2">
        ...
        {warn != null && <span className="flex items-center gap-1.5"><i className="w-3 h-px" style={{ background: 'var(--warn)' }} />Alerta</span>}
        {crit != null && <span className="flex items-center gap-1.5"><i className="w-3 h-px" style={{ background: 'var(--danger)' }} />Crítico</span>}
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ minHeight: 260, cursor: 'crosshair' }}
        ...
        {yTicks.map((tick) => (
          <g key={tick}>
            ...
            <text x={pad.left - 10} y={y(tick) + 3} textAnchor="end" fill="var(--text-dim)" fontSize="10">{fmt(tick, unit)}</text>
          </g>
        ))}

        {/* Health thresholds — dashed guides the ADWIN signal is read against. */}
        {warn != null && Number.isFinite(warn) && (
          <line x1={pad.left} x2={width - pad.right} y1={y(warn)} y2={y(warn)} stroke="var(--warn)" strokeWidth={1} strokeDasharray="5 4" opacity={0.55} />
        )}
        {crit != null && Number.isFinite(crit) && (
          <line x1={pad.left} x2={width - pad.right} y1={y(crit)} y2={y(crit)} stroke="var(--danger)" strokeWidth={1} strokeDasharray="5 4" opacity={0.55} />
        )}
        ...
        {drifts.map((item) => {
          ...
          return (
            <g key={item.index} onClick={(event) => { event.stopPropagation(); onSelectDrift(item.index) }} style={{ cursor: 'pointer' }}>
              ...
              {!isSelected && <line x1={dx} x2={dx} y1={pad.top} y2={height - pad.bottom} stroke="var(--danger)" strokeWidth={1} opacity={0.2} />}
              <path d={`M ${dx - 4} ${pad.top - 9} L ${dx + 4} ${pad.top - 9} L ${dx} ${pad.top - 1} Z`}
                fill={isSelected ? 'var(--crit-2)' : 'rgba(220,38,38,0.5)'} />
            </g>
          )
        })}
        ...
        {points.map((point, i) => outliers.has(point.index)
          ? <circle key={point.index} cx={x(i)} cy={y(point.v)} r="2.4" fill="var(--warn)" opacity="0.72" />
          : null)}
        ...
        {selectedPoint != null && (() => {
          ...
          return point ? (
            <g>
              <line x1={x(i)} x2={x(i)} y1={pad.top} y2={height - pad.bottom} stroke="var(--text-strong)" opacity="0.45" />
              <circle cx={x(i)} cy={y(point.v)} r="5" fill="var(--text-strong)" stroke="var(--graph-node-strong)" strokeWidth="2" />
            </g>
          ) : null
        })()}
        ...
        {drift && (() => {
          ...
          return (
            <g pointerEvents="none">
              {startI >= 0 && <line x1={x(startI)} x2={x(startI)} y1={pad.top} y2={height - pad.bottom} stroke="var(--warn)" strokeWidth={1.5} strokeDasharray="3 4" />}
              <line x1={dx} x2={dx} y1={pad.top} y2={height - pad.bottom} stroke="var(--crit-2)" strokeWidth={2} />
              <g transform={`translate(${labelX},${pad.top + 3})`}>
                <rect x={-31} y={0} width={62} height={15} rx={3} fill="var(--danger)" />
        ...
        {xTicks.map((tick) => (
          <text key={tick} x={x(tick)} y={height - 18} textAnchor="middle" fill="var(--text-dim)" fontSize="10">
            {points[tick]?.t ? fmtDate(points[tick].t) : tick}
          </text>
        ))}
        <text x={width / 2} y={height - 4} textAnchor="middle" fill="var(--muted-foreground)" fontSize="10">Leituras em ordem cronológica</text>
      </svg>
    </div>
  )
}
